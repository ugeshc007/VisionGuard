...
function enrichCameraStream(camera = {}) {
    ...
    return {
        ...
        // WHEP-style endpoint: POST an SDP offer, get an SDP answer back. Preferred over
        // hlsUrl for browser playback — go2rtc's live HLS remux of the transcoded stream
        // reliably throws a fatal hls.js fragParsingError, whereas WebRTC uses the browser's
        // native RTP/H.264 decode path and actually renders frames. Routed through our own
        // backend (not go2rtc directly) because go2rtc's /api/webrtc doesn't send CORS
        // headers on its OPTIONS preflight, which browsers require for a cross-port POST.
